    ...
    <sticky v-if="!isLook" className="editor-topbar" :stickyTop="78">
      ...
      <div class="topbar-action">
        <el-button-group>
          ...
          <el-button :disabled="!this.questions.lists.length" @click="saveQuestions('btn')" type="primary">保存至草稿箱</el-button>
          <template v-if="!this.questions.lists.length">
            <el-button :disabled="!this.questions.lists.length" type="primary" icon="el-icon-view">预览</el-button>
          </template>
          <template v-else>
            <router-link :to="{name: 'preview-submit', params: {id: this.id}}" target="_blank">
                          ...
                          <template v-if="list.iType ==='radio' || list.iType ==='checkbox'">
                            <div class="options-list">
                              ...
                              <ul v-if="addInput[index+1]" class="options-other">
                                <li class="list-item">
                                  <div class="option-input-wrap">
                                    <div :id="`${index+1}_addInput`" class="inline-editor" contenteditable="true">
                                      <template v-if="editorData[index+1][`${index+1}_addInput`]">
                                        <div v-html="editorData[index+1][`${index+1}_addInput`]"></div>
                                      </template>
                                      <template v-else>
                                        <div><p>其他</p></div>
                                      </template>
                          ...
                          <template v-if="list.iType === 'radio' || list.iType === 'checkbox'">
                            <div
                              ...
                              <textarea
                                ...
                                :placeholder="`${option.placeholder}（${list.iRuleOther.required? '必填':'选填'}-不得超过${list.iRuleOther.maxlength}字）`"
                                rows="3"></textarea>
                            </div>
                          </template>
                          <template v-else-if="list.iType.split('_')[0] === 'matrix'">
...
export default {
  ...
  methods: {
    ...
    logicActiveReset() {
      this.logicActive = {}
      if (!Array.isArray(this.logicData.iOptions)) return false
    ...
    getDataEditor(num, status) {
      for (let [tempName, instance] of Object.entries(this.CKEDITOR.instances)) {
        ...
        if (this.editorCom.indexOf(tempName) === -1) {
          ...
          this.editorData[num][tempName] = tempData
        } else {
          this.editorData[tempName] = tempData
        }
      }
      console.log(this.editorData)
    },
